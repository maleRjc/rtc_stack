...
is_debug=false
rtc_build_ssl=false
rtc_ssl_root="/home/build/rtc_stack/build/libdeps/build/include"
END
)
...
build_only(){
  ...
  gn gen out --args="$GN_ARGS"
  ninja -C out call default_task_queue_factory
  all=`find ./out/obj/ -path "./out/obj/third_party/yasm" -prune -o -type f -name "*.o"`
  all=`echo $all | sed "s/\.\/out\/obj\/third_party\/yasm//g"`
  if [[ -n $all ]];
  then
    rm -f ../libwebrtc.a
    ar cq ../libwebrtc.a $all
