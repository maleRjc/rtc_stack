...
#include "api/i420_buffer.h"
#include "api/video_bitrate_allocator_factory.h"
#include "api/video_codec_constants.h"
#include "api/video_encoder.h"
#include "video/vp9/svc_rate_allocator.h"
#include "video/video_codec_initializer.h"
#include "video/default_video_bitrate_allocator.h"
#include "rtc_base/arraysize.h"
#include "rtc_base/checks.h"
#include "rtc_base/alr_experiment.h"
#include "rtc_base/quality_scaling_experiment.h"
...
class VideoStreamEncoder::VideoSourceProxy {
 public:
  ...
  bool RequestResolutionLowerThan(int pixel_count,
    ...
    sink_wants_.target_pixel_count = std::nullopt;
    source_->AddOrUpdateSink(video_stream_encoder_,
                             GetActiveSinkWantsInternal());
    return true;
  }
...
GetEncoderBitrateLimits(const VideoEncoder::EncoderInfo& encoder_info,
  ...
  for (size_t i = 0; i < bitrate_limits.size(); ++i) {
    ...
    if (bitrate_limits[i].frame_size_pixels >= frame_size_pixels) {
      return std::optional<VideoEncoder::ResolutionBitrateLimits>(
          bitrate_limits[i]);
    }
  }
  ...
  switch (reason) {
    case kCpu:
      if (!IsFramerateScalingEnabled(degradation_preference_))
        counts.num_framerate_reductions = std::nullopt;
      if (!IsResolutionScalingEnabled(degradation_preference_))
        counts.num_resolution_reductions = std::nullopt;
      break;
    case kQuality:
      if (!IsFramerateScalingEnabled(degradation_preference_) ||
          !quality_scaler_) {
