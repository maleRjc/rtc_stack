...
class Call final : public webrtc::Call,
 ...
 private:
  ...
  bool aggregate_network_up_ = false RTC_GUARDED_BY(configuration_sequence_checker_);
  ...
  //RtpStateMap suspended_audio_send_ssrcs_ RTC_GUARDED_BY(configuration_sequence_checker_);
  //RtpStateMap suspended_video_send_ssrcs_ RTC_GUARDED_BY(configuration_sequence_checker_);

 //using RtpPayloadStateMap = std::map<uint32_t, RtpPayloadState>;
 //RtpPayloadStateMap suspended_video_payload_states_ RTC_GUARDED_BY(configuration_sequence_checker_);
  ...
  std::optional<int64_t> first_received_rtp_audio_ms_;
  std::optional<int64_t> last_received_rtp_audio_ms_;
  ...
  uint32_t last_bandwidth_bps_ = 0 RTC_GUARDED_BY(&last_bandwidth_bps_crit_);
  // TODO(holmer): Remove this lock once BitrateController no longer calls
  // OnNetworkChanged from multiple threads.
  rtc::CriticalSection bitrate_crit_;
  uint32_t min_allocated_send_bitrate_bps_ = 0
      RTC_GUARDED_BY(&worker_sequence_checker_);
  uint32_t configured_max_padding_bitrate_bps_ = 0 RTC_GUARDED_BY(&bitrate_crit_);
...
void Call::SetClientBitratePreferences(const BitrateSettings& preferences) {
  RTC_DCHECK_RUN_ON(&configuration_sequence_checker_);
  transport_send_ptr_->SetClientBitratePreferences(preferences);
}
...
webrtc::VideoReceiveStream* Call::CreateVideoReceiveStream(
  ...
  VideoReceiveStream* receive_stream = 
    new VideoReceiveStream(task_queue_factory_, 
                           ...
                           num_cpu_cores_,
                           transport_send_ptr_->packet_router(), 
